...
%% You can set the chamber and reservoir pressure, although the nozzle has to be the same for every run.
    ...
    Mesh_Density_Factor = 0.25;
    CFD_Iterations = 500;
    Pseudo_Transient_Timescale_Factor = 0.5;
    %Define the inlet/outlet conditions 
    Tin = 300; % Inlet Temp BC (K) - Reservoir Temp
    Tout = 300 ; % Outlet Temp BC (K) - Vaccum Line Temperature (same as reservoir usually)
    ...
    Reactor_Length = 774; %mm -- Bham = 1000mm, Leeds = 774mm
    Reactor_Radius = 120; %mm -- Bham = 198.2mm, Leeds = 120mm
    ...
    Outlet_Position = 550; %mm % This is the position of the outlet from the left most wall 875mm for Bham, 550mm for Leeds
    
    Contour = 0; % 0 for no contour plot and 1 for 2D contour plot, note 2D contour plot takes a while to output
    Y_Sampling_Interval = 30; % This sets the y range for the contour plot in mm. If this is 30, you will get contour data from 0 - 30mm away from the axis.
