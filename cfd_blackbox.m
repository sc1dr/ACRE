...
function cfd_blackbox(Solution_Folder, CPU_Cores, Ansys_Version, Tin, Tout, Pin,Pout,Bath_Gas, Reactor_Length, Reactor_Radius, ...
    Reservoir_Radius,Reservoir_Length, Reservoir_Inlet_Diameter, Outlet_Diameter, Outlet_Position,Contour,Y_Sampling_Interval, Solver_Type, Mesh_Density_Factor, Pseudo_Transient_Timescale_Factor, ...
    CFD_Iterations)
...
%% Setting up folders and using nozzle data (do not touch)
    ...
    Contour_Line = ((Y_Sampling_Interval/1000)/0.0002) + 1; % takes sampling intervals and tells ansys how many lines it needs to make at 0.2mm spacing, i.e. if y = 30 it will make 150 lines.
    ScriptChanger(FluentScriptFile,73, ['    ((>= i ',  num2str(Contour_Line), '))'])
    ScriptChanger(FluentScriptFile,77, ['    (ti-menu-load-string (format #f "/surface/line-surface line_~a ', num2str(Nozzle_Length/1000), ' ~a ',num2str(Reactor_Length/1000), ' ~a" ycoord ycoord ycoord))'])
    ...
    if Contour == 1
        ...
        ScriptChanger(FluentScriptFile,73, ['    ((>= i ',  num2str(Contour_Line), '))']);
        ScriptChanger(FluentScriptFile,74,  '    (define a 5000)');
    ...
    Nozzle_inlet_index = max(find(abs(axial_position-Nozzle_Length)<0.0001));

    length_idx = find(axial_position - 0 < 0.01);
    length_idx_2 = find(axial_position - 0.05 < 0.01);
    t_avg_in_idx = mean(axial_temp_data(length_idx(end):length_idx_2(end)));
    t_std_in_idx = std(axial_temp_data(length_idx(end):length_idx_2(end)));
    temp_cutoff = t_avg_in_idx + t_std_in_idx;

    cutoff_index = find(axial_temp_data - temp_cutoff < 0.1);
    Flow_Breakdown = axial_position(cutoff_index(end))*100;
    Flow_Breakdown_Index = cutoff_index(end)
    %Flow_Breakdown_Index = find(axial_position*100 == Flow_Breakdown)
    Throat_x_idx = max(find(axial_position < Throat_Radius) < 0.001);
	...
	mkdir([Solution_Folder, '/Contour_Data'])
        ...
        y_interval = 0.0002; % y interval in m (0.2mm) (1/5000 in CFD macro)
        ...
        for i = 1:Contour_Line
            ...
            data_x = data_t(:, 1) - (Nozzle_Length/1000); % nozzle lengh is in mm , data in m
            data_x = rmmissing(data_x);
            y_value = (i - 1) * y_interval;
	        ...
	        data_x = downsample(data_x,20)*100;
	        data_y = downsample(data_y,20)*100;
	        data_t = downsample(data_t,20);
            data_d = downsample(data_d,20);
	        data_p = downsample(data_p,20);
            data_m = downsample(data_m,20);
